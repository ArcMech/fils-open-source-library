import {
  ...
  export const mapToArray = data =>
    isArray(data) ? data : Object.keys(data).map(key => [key, data[key]])
  ...
  export const intersection = (obj, defaultValues) =>
    pick(defaults(omitBy(obj, isNull), defaultValues), Object.keys(defaultValues))
  
  export const calculateMonthlyCost = ({ amount, months, rate }) => {
  ...
  export const fixNumbers = num => (Math.round(num * 100) / 100).toFixed(2)
  
  // Turns document file name into document name
